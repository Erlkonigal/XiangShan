...
//  XSError(ifuWbPtr < commitPtr && !isFull(ifuWbPtr(0), commitPtr(0)), "ifuWbPtr runs ahead of commitPtr")
  ...
  private val specTopAddr = speculationQueue(io.fromIfu.pdWb(0).bits.ftqIdx.value).topRetAddr.toUInt
  private val ifuRedirect = receiveIfuRedirect(io.fromIfu.pdWb(0), specTopAddr)
